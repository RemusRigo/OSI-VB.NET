...
Public Class frmBaseBoard
         ...
         For Each obj As ManagementObject In objItems
            ...
            If obj.Properties.Cast(Of PropertyData)().Any(Function(p) p.Name = "Product") Then
               If (obj("Product") IsNot Nothing) Then
                  items.Add(New ProcListItem With {.Group = groupName, .Label = "Product", .Value = obj("Product").ToString(), .ImageIndex = 0})
               End If
            End If
            worker.ReportProgress(0, New ProgressInfo With {.Max = totalProps, .Value = crtAction}) : crtAction += 1
            ...
            If obj.Properties.Cast(Of PropertyData)().Any(Function(p) p.Name = "Version") Then
               If (obj("Version") IsNot Nothing) Then
                  items.Add(New ProcListItem With {.Group = groupName, .Label = "Version", .Value = obj("Version").ToString(), .ImageIndex = 0})
               End If
            End If
            worker.ReportProgress(0, New ProgressInfo With {.Max = totalProps, .Value = crtAction}) : crtAction += 1
            ...
            If obj.Properties.Cast(Of PropertyData)().Any(Function(p) p.Name = "Tag") Then
               If (obj("Tag") IsNot Nothing) Then
                  items.Add(New ProcListItem With {.Group = groupName, .Label = "Tag", .Value = obj("Tag").ToString(), .ImageIndex = 0})
               End If
            End If
            worker.ReportProgress(0, New ProgressInfo With {.Max = totalProps, .Value = crtAction}) : crtAction += 1
            ...
            If obj.Properties.Cast(Of PropertyData)().Any(Function(p) p.Name = "HostingBoard") Then
               If (obj("HostingBoard") IsNot Nothing) Then
                  items.Add(New ProcListItem With {.Group = groupName, .Label = "Hosting Board",
            ...
            If obj.Properties.Cast(Of PropertyData)().Any(Function(p) p.Name = "HotSwappable") Then
               If (obj("HotSwappable") IsNot Nothing) Then
                  items.Add(New ProcListItem With {.Group = groupName, .Label = "Hot Swappable",
                    .Value = If(obj("HotSwappable"), "Package can be hot-swapped", "Package can't be hot-swapped"), .ImageIndex = 0})
               End If
            End If
            worker.ReportProgress(0, New ProgressInfo With {.Max = totalProps, .Value = crtAction}) : crtAction += 1
            ...
            If obj.Properties.Cast(Of PropertyData)().Any(Function(p) p.Name = "Removable") Then
               If (obj("Removable") IsNot Nothing) Then
                  items.Add(New ProcListItem With {.Group = groupName, .Label = "Removable",
                     .Value = If(obj("Removable"), "Package is removable", "Package is not removable"), .ImageIndex = 0})
               End If
            End If
            worker.ReportProgress(0, New ProgressInfo With {.Max = totalProps, .Value = crtAction}) : crtAction += 1
   ...
   Private Sub BackgroundWorker_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs)
      ...
      If e.Result IsNot Nothing Then
         ...
         If lvBaseBoard.InvokeRequired Then
            lvBaseBoard.Invoke(New Action(Of List(Of ProcListItem))(AddressOf UpdateListView), items)
         Else
